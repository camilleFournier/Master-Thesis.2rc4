...
%\todo[color=cyan!20]{Also introduce smoothness as it is the main part of the thesis}
\iffalse
\todo[inline, color=green!40]{Intro is too short.
Please add: 
- a part of novelty (what is the state of the art and how novel is your contribution)
- a part about the methodology and key results

Each part can be short (one or 2 paragraphs) and focused on the essential aspects}
\fi
...
\end{itemize}
\todo[inline, color=green]{Need to elaborate on the last paragraph of this page. The global challenge is to collect relevant performance metrics. For this, you need two things: the ability to run different versions of an application, in a controlled environment; tools that monitor performance metrics.
We address the first challenge with [here, explain which apps you use, which workload, etc.]
To address the second challenge, we investigate several techniques and tools. [One sentence to explain what you investigate to measure memory perf.; another sentence for CPU metrics; then elaborate on the absence of a known framework to measure the smoothness of PWAs]
The key technical contribution of this thesis consists in the elaboration of a relevant model to assess the smoothness of PWAs. [Explain what you did to build this model, and what is the form of the model, how it can be reused by other developers and researchers, and how you use it in your thesis] }
...
We address the first challenge with 3 applications: a Native Android app, a cross-platform app developed with React Native and a Progressive Web App developed with ReactJS. \todo[color=cyan!20]{What does he mean, which workload?}
...
%\todo[inline]{Rephrase the research question}
...
%\todo[inline]{Extend with a summary of methodology, results and discussion}

\chapter{Background}

%\todo[inline, color=green]{Restructure: start with 2.2 (a generic intro to mobile dev) then 2.1 (PWA), 2.3(Profiling tools), 2.4(Rendering pipeline). Extract the state of the art as a separate section of the chapter, which comes just before the problem statement. The flow is as follows: introduce the context, the technical scope of your work, current state of the art and conclude with gaps in the state of the art and the statement of your research problem.}
...
\iffalse
\todo[inline, color=green]{I suggest to restructure this section
- select the closest works to your study, discuss them in detail and add an explicit part where you discuss the limitations of these works and how your work addresses these limitations and contributes to the state of the art
- then, summarize the other works in a couple of paragraphs, grouped by topics (for example resource analysis for PWA, user experience, etc.)
- it would also be good to cite some related works in the area of performance evaluation}
\fi
...
%Khan et al. \cite{pwa_ahp} compared native, mobile web, hybrid and progressive web apps using an Analytical Hierarchy Process with three criteria : application size, multi-platform support and offline accessibility. They concluded \todo[color=green!40]{what does that mean?} that Progressive Web Apps were the best mobile development option. \newline
...
%\todo[inline, color=cyan!20]{Change definition of smoothness and rephrase according to introduction}
...
%\todo[inline, color=green!40]{have a specific section where you elicit each question, motivate them, and explain the methodology to answer them}
...
\iffalse
We will reach this goal by answering 3 questions: How can we compare the smoothness of Progressive Web Applications and Mobile Applications? Are Progressive Web Applications as smooth as Mobile Applications? How efficient are they, that is how many resources are used to render a smooth application? \newline
...
\label{method:smoothness}
    ...
    The smoothness of an application according to Android and Chrome is closely related to the frames displayed by the application\citationneeded. If a frame takes too long to be rendered, the user might notice some stuttering or freezing\todo[color=green]{explain stuttering and freezing}, resulting in a poorer quality of the User Experience. Thus, comparing the smoothness of Mobile applications and PWA amounts to compare the frame duration\todo[color=green]{frame display duration ?} on Android and the browser, Chrome in this study. Hence, understanding how native Android and Chrome compute frames, for an executing application, is essential. 
...
As was presented in the background, only a few Android emulators do not enhance the hardware capabilities of the emulated device \todo[color=green]{What does that mean?}. Only 3 were identified and tested: Android Emulator, Genymotion and Visual Studio Android Emulator. As was explained previously, the metrics used to compare the performance of the emulators were the CPU usage using \textit{dumpsys cpuinfo}, the total RAM used using \textit{dumpsys meminfo} and the percentage of janky frames using \textit{dumpsys gfxinfo}. The measures were taken while the devices ran the same application that changed a text every few milliseconds. No human interaction was necessary. The results presented in \autoref{tab:emulators_test} are the average of 110 measurements.
...
%\todo[inline, color=green]{It is important that the RQs appear here and that the results answer the RQs}
...
%This chapter will present the results of the experiments described in the previous part. It includes the empirical model of Chrome's rendering pipeline, the smoothness metric derived from it, the results of the experiments regarding CPU usage and lastly, the smoothness performance of Mobile Applications and Progressive Web Apps.

%\todo[inline, color=green!40]{this preliminary experiment to select the device can be part of the previous chapter}

%\todo[inline]{Present the results in the same order that the RQ are formulated}

%\todo[inline]{Write the takeaways explicitly for each research question.}
...
%\todo[inline]{Memory is first than CPU in the RQs}
...
\iffalse
The first observation is the difference of performance of the same application between devices. In both Scrolling scenarios, the Native and Hybrid applications takes 3 or 4 times more time to compute a frame on Samsung S6 than on Samsung S5 and Huawei. The difference is less significant on Changing Scenarios though a two-times magnitude difference can still be observed. However, the progressive Web App offers a more consistent smoothness between devices.
\paragraph{}
...
\iffalse
\paragraph{}
The second observation is the difference observed between the scenarios in the Hybrid and Progressive Web applications, though the rank of the scenarios is not consistent between devices. The Hybrid application consumes the most memory on Samsung S5 when scrolling a text, and on Samsung S6 and Huawei P9-Lite when scrolling pictures. The Progressive Web App consumes the most when changing pictures on Samsung S6 and Huawei P9-Lite, and when scrolling (either text or pictures) on Samsung S5.  
