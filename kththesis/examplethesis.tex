...
%At the time of writing, Progressive Web App are not fully supported on iOS \cite{BackgroundSync_support} \cite{Manifest_support}. Thus, this work will focus on comparing PWA and traditional mobile applications only on Android. 
...
\todo[color=magenta]{I would suggest adding some figures to better show the results}
\paragraph{}
...
%This way is itself divided in several different approaches that share a common characteristic: the use of the same code to build the application for several platforms. 
...
%\todo[inline]{Add numbers to the components and refer them in the text}
...
%\todo{What do you mean with the next sentence. I think you should reprahse it} 
%maybe even pre-painting, painting, tiling and rastering if it was already done.
...
%\todo[inline, color=cyan!20]{Done. (I think)}
...
%This chapter will present the methods used to compare the smoothness performance of Progressive Web Apps and Mobile Applications on Android.
%This includes how the smoothness metric was defined and the experiments conducted to compare this metric. \todo{Extend a little bit, one sentence per subsection, two at most. A reader can get missed jumping from Smoothness metric to Memory and CPU}

%This includes the metric used to compare the smoothness of the applications, the tools used to measure this metric and the resources used as well as the experiments conducted to answer the research question. 


%\todo[inline]{It is a good idea to mention the research questions again. }
...
\label{method:smoothness}
    ...
    %\subsection{Android smoothness metric}
...
%\todo{Add in parenthesis or footnote what is a "janky" frame}
    ...
    %\todo{Reference to the figure itself}
    \paragraph{}
...
%\todo{Take a look to the subsection declaration to see the example about how to create cross references :)}
    ...
    The model was first built with a single benchmark application with limited interaction. It was then confronted with 10 traces of 8 different PWAs from the Github repositories pwarocks\footnote{https://github.com/pwarocks/pwa.rocks} and awesome-pwa\footnote{https://github.com/hemanth/awesome-pwa} to include a wider range of interactions\todo{From where and why?}. This confrontation was done 3 times each, with new PWAs until no significant errors remained.
    ...
    %\subsection{Chrome smoothness metric}
...
%\todo[inline]{Try to avoid orphans, or lonely sentences.}
...
\section{Resources}
    ...
    %\todo[inline]{I think is good enough}
    %\subsection{Android}
    %\subsection{Chrome}
...
\paragraph{}
\todo[inline, color=cyan!20]{Change if I don't look at graphic buffers}
...
%The experiments conducted to evaluate the CPU sampling method of Chrome were run a hundred times for each method and CPU workload. According to the results presented in \autoref{fig:cpu_usage}, the CPU sampling method of Chrome is not accurate enough. If it was, its curve would be close to the measures taken by dumpsys (\todo{mention specifically which curves, "orange line", "dashed blue line", etc}), though always behind as \textit{dumpsys cpuinfo} provides a maximum CPU usage of the application. This is not the case here. 



\todo[inline]{We need to talk a little bit more about this}
...
%\todo[inline]{Is not good to have orphans :). Write some sentences here about the section}
...
%\todo[inline]{Short repetitions to enter again into the context. For example, "Following the methdology proposed in ... we formulate and describe  the following model as the one that Chrome uses to render contents in Android devices."}
...
%\todo[inline, color=cyan!20]{Compute avg size of graphic buffer size which seems to explode when changing picture}
...
%This chapter will present the conclusions that can be drawn from the results as well their limitations.
...
%\paragraph{}
%Because of the limited access to Android devices, the use of Android emulators was considered and their performance evaluated. 
...
%\todo[inline]{Limitations as a section}
\fi

%\todo[inline]{Future work: Add as a separated chapter or merge with the conclusions one}
...
%\todo[inline]{Create two sections, "contributions" and "future research" (move from previous chapter). In the contribution section, summarize each research question in one paragraph}
%The main research question of this thesis, are Progressive Web Apps as efficient as Mobile Applications in terms of smoothness, was partially answered. The limited devices, frameworks and OS used for this study prevent us to reach a true conclusion. Future work that extends this thesis to other devices, frameworks and OS would be needed to do so.
