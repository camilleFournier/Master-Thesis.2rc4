...
Johannsen \cite{JohannsenFabian2018PWAa} evaluated the code complexity brought by Progressive Web Applications to a regular application using different metrics such as the cyclomaic complexity and Halstead effort. He concluded that the added complexity was low\todo[color=green!40]{how is this assessed?}. \newline
...
\label{method:smoothness}
    ...
    %However, Chrome Tracing tool can record and display many functions called by Chrome's processes.
    
    \todo[inline, color=green!40]{this model is part of your contributions, right?
Make it more explicit, for example replace the first sentence by: The first step of our methodology consisted in building a model of chrome graphics pipeline, through an iterative process.
Aslo, you need to motivate why you build such a model. How does it help to answer the RQs? }
    
    \paragraph{}
    To this end, we used Chrome Tracing tool which can record and display many functions called by Chrome's processes. Th model was built empirically with an iterative process. The events recorded by Chrome Tracing tool were processed by a script which tracked the start and end of the frames as predicted by the model. If any error was raised, a visualization of the events on Chrome Tracing tool and a revision of the code allowed either to fix a bug in the script\todo[color=green!40]{What code?} or to change the model according to the events observed. \newline
